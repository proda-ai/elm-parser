module Compat.Parser.Advanced exposing
    ( Parser, run, DeadEnd, inContext, Token(..)
    , int, float, number, symbol, keyword, variable, end, ignorer, keeper
...
@docs int, float, number, symbol, keyword, variable, end, ignorer, keeper
...
keyword (Token kwd expecting) =
    ...
    Parser <|
        \s ->
            ...
            in
            if newOffset == -1 || 0 <= isSubChar (\c -> (Char.isUpper c || Char.isLower c || Char.isDigit c) || c == '_') newOffset s.src then
...
consumeBase =
    Native.Compat.Parser.consumeBase


consumeBase16 : Int -> String -> ( Int, Int )
consumeBase16 =
    Native.Compat.Parser.consumeBase16
...
finalizeFloat invalid expecting intSettings floatSettings intPair s =
    ...
    if floatOffset < 0 then
        ...
        case floatSettings of
            ...
            Ok toValue ->
                case String.toFloat (String.slice s.offset floatOffset s.src) |> Result.toMaybe of
                    Nothing ->
                        Bad True (fromState s invalid)
...
chompBase10 =
    Native.Compat.Parser.chompBase10
...
chompUntilEndOr str =
    Parser <|
        \s ->
            let
                ( newOffset, newRow, newCol ) =
                    Native.Compat.Parser.findSubString str s.offset s.row s.col s.src
...
isSubString =
    Native.Compat.Parser.isSubString
...
isSubChar =
    Native.Compat.Parser.isSubChar
...
isAsciiCode =
    Native.Compat.Parser.isAsciiCode
...
findSubString =
    Native.Compat.Parser.findSubString
